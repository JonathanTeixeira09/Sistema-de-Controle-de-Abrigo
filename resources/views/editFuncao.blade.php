...
@section('conteudo')
        ...
        <div class="col-md-8  mx-auto justify-content-center align-items-center flex-column">
            ...
            <form class="row g-2" action="{{ route('updatefuncao', $funcaos->id) }}" method="POST" name="formCadastro" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="col-md-12">
                    <label for="nome" class="form-label">Nome da Função</label>
                    <input type="text" class="form-control @error('nome') is-invalid @enderror" name="nome" placeholder="Enfermeiro" tabindex="1" value="{{ $funcaos->nome }}">
                    @error('nome')
